...
const ContentProduct = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://161.132.40.129/api/product');
      if (!response.ok) {
  ...
  const fetchDataSelect = async () => {
    try {
    const response_category = await fetch('http://161.132.40.129/api/category');
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await fetch(`http://161.132.40.129/api/product/${id}`, {
            method: 'DELETE',
          });
  ...
  const ConnectEdit = async (newProduct ) => {
    try {
         ...
         const response = await fetch(`http://161.132.40.129/api/product/${newProduct.id}`, {
            method: 'PUT',
            headers: {
...
try {
  ...
  const response = await fetch(`http://161.132.40.129/api/product/ObteinSku/${event.target.value}`);
  const valor = await response.json();
  setNewProduct((prevProduct) => ({
    ...prevProduct,
    'sku': valor.data,
