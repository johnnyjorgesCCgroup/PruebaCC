...
const ContentSubCategory = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://161.132.40.129/api/subcategory');
      if (!response.ok) {
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await fetch(`http://161.132.40.129/api/subcategory/${id}`, {
            method: 'DELETE',
          });
  ...
  const ConnectEdit = async (newSubCategory ) => {
    try {
      console.log(newSubCategory, categoryId);
         ...
         const response = await fetch(`http://161.132.40.129/api/subcategory/${newSubCategory.id}`, {
            method: 'PUT',
            headers: {
